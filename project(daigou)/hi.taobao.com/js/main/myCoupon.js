$(function() {
	...
	//选择优惠券进行抵扣
	$(".user-coupon ul").on("tap", "li", function() {
		...
		if(!flag) {
			if(isCanUse >= isMoney) {
				...
				window.location.href = "../shopping/affirmOrderForm.html"
			} else {
				alert("不满使用条件")
			}
		} else {
	...
	function autoLanguage() {
		...
		if(localStorage.getItem("language") == "0") {
			...
			$(".lang_coupon_1").text("My Discount Coupon")
			$(".lang_coupon_2").text("Usable");
			$(".lang_coupon_3").text("Expired");
			$("head link:last-child").remove();
			couponUSA();
		} else if(localStorage.getItem("language") == "2") {
			$(".lang_coupon_1").text("Kupon Saya");
			$(".lang_coupon_2").text("Boleh digunakan");
	...
	function couponUSA() {
		var link = '<link rel="stylesheet" type="text/css" href="../../css/language/coupon_USA.css"/>';
		$("head").append(link);
	}

	function couponMalaysia() {
		var link = '<link rel="stylesheet" type="text/css" href="../../css/language/coupon_Malaysia.css"/>';
		$("head").append(link);
	}
});